...
example structure below, with `ewc-ansible-role-htcondor-execute` being a clone of this
repository:
```
.
├── roles
│   └── ewc-ansible-role-htcondor-execute
├── inventory.yml
└── playbook.yml
```
...
# playbook.yml
---
- name: Install HTCondor Execute
  hosts: htcondor_execute
  become: true
  become_user: root
